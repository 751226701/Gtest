...
* @param:	HANDLE_T instance		ITA实例。
* @param:	short y16				Y16值。
* @param:	ITAAnalysisParam* param	分析对象可以调节的参数结构体。
* @param:	float * surfaceTemp		输出表面温度。
* @return:	返回值类型：ITA_RESULT。成功，返回ITA_OK；失败，返回值<0，参考ERROR.h。
**/
ITA_API ITA_RESULT ITA_MeasureAnalysisTempPoint(HANDLE_T instance, short y16, ITAAnalysisParam* param, float* surfaceTemp);
...
* @return:	返回值类型：ITA_RESULT。成功，返回ITA_OK；失败，返回值<0，参考ERROR.h。
**/
ITA_API ITA_RESULT ITA_MeasureAnalysisTempMatrix(HANDLE_T instance, short* y16Array, int y16W, int y16H,
	int x, int y, int w, int h, ITAAnalysisParam* param, float* tempMatrix, int matrixSize);
...
* @param: ITA_CALIBRATE_RESULT *result		输出校温结果。
* @return: 返回值类型：ITA_RESULT。成功，返回ITA_OK；失败，返回值<0，参考ERROR.h。
**/
ITA_API ITA_RESULT ITA_StartCalibrate(HANDLE_T instance, float *kf, float *b, ITA_CALIBRATE_RESULT*result);
...
* @param:	int *isChange 		    输出参数，切换后的档位。1：高温档：工业低温，工业高温等等; 0：低温档：人体测温。
* @return:	返回值类型：ITA_RESULT。成功，返回ITA_OK；失败，返回值<0，参考ERROR.h。
**/
