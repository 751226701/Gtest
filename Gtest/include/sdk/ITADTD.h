...
#ifndef ITA_API
#   if defined(_MSC_VER) && !defined(ITA_SATICLIB)
#       if defined(ITA_SHAREDLIB)
#           define ITA_API __declspec(dllexport)
#       else
#           define ITA_API __declspec(dllimport)
#       endif
#   else
#       if defined(ITA_SHAREDLIB) && defined(ITA_HIDDEN_VISIBILITY)
#           define ITA_API __attribute__((visibility("default")))
#       else
#           define ITA_API
#       endif
#   endif
...
typedef enum ITADCType
{
	...
	ITA_DETECTOR_FRAME_FREQUENCY   //设置探测器帧频，范围1-50，单位fps。建议使用以下典型帧频：9、15、20、25、30、50。默认25。参数类型：整型。
...
typedef enum ITABCType
{
	...
	ITA_SET_OpenMPAccelerateCore,//设置OPENMP加速的核心数，结合ITA_GetCurrentAlgorCostTime确定最佳核心数。参数类型：整形。
...
typedef enum ITAMCType
{
	...
	ITA_Y16_OFFSET				//y16偏移量。默认值为0。参数类型：short。
...
typedef enum ITAICType
{
	...
	ITA_AUTO_RPBDPS,		//自动去坏点，参数类型：unsigned char。取值范围是0，1.
...
typedef enum ITADebugType
{
	...
	ITA_SET_ALGORCOSTTIME		//设置是否开启单帧算法耗时统计。参数类型：bool。false,关闭算法耗时统计。true,打开算法耗时统计。
...
typedef struct ITACalibrateResult
{
	int BlackBodyNumber;
	float TBlackBody[10];
	short Y16[10];
...
typedef struct ITA_MEASURE_INFO
{
	...
	unsigned char          bTransCorrection;//窗口透过率校正开关
	unsigned char          bHumidityCorrection;//湿度校正开关
	unsigned char		   bShutterCorrection;//快门修正开关
	unsigned char		   bLensCorrection;//镜筒修正开关
	unsigned char		   bAtmosphereCorrection;//大气透过率修正开关
...
typedef enum ITAShapeType 
{
    ...
    ITA_SHAPE_TYPE_POLYGON  = 7,  // 手绘多边形 (每个顶点的坐标,顺时针方向,不允许存在线宽)
    ITA_SHAPE_TYPE_CIRCLE   = 8,  // 圆形 (传入外接矩形的左上和右下顶点坐标)
    ITA_SHAPE_TYPE_ELIPSE   = 9,  // 椭圆 (水平放置的椭圆,传入椭圆外接矩形的左上和右下顶点坐标)
    ITA_SHAPE_TYPE_CONTOUR  = 10  // 手绘多边形 (顶点坐标为轮廓上的点,与POLYGON不同的是此种图形的边允许存在线宽)
...
typedef struct ITA_AnalysisParam {
	float  fEmiss;			//分析对象发射率
	float  fDistance;		//分析对象距离
	float  fHumidity;		//分析对象湿度
	float  fReflectTemp;	//分析对象反射温度
}ITAAnalysisParam;
